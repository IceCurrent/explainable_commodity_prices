...
On the aligned daily sample (**T=2691**, K=5 vanilla-AE latent factors, J=37 macro variables), the spanning question is settled by the **out-of-sample (purged-CV) canonical correlations vs the circular-shift permutation null** — not the in-sample numbers, which a J=37 collinear panel inflates by construction.

- **ρ_min**: in-sample 0.144, **OOS 0.055**, perm-null p95 0.097, **perm-p 0.0010**, bootstrap 95% CI [0.143, 0.219].

- **ρ_mean**: in-sample 0.378, OOS 0.299, perm-null p95 0.126, perm-p 0.0010.

- **2 of 5** factor directions are macro-spanned at significance (OOS ρ>0.3 and perm-p<0.05). ρ_min is the all-five-spanned statistic; clearing the (high) null band is what counts, not the raw level.

- Note: the per-dimension `perm-p` is computed against the *in-sample* ρ (necessary significance, not sufficient). The weak dims can be in-sample significant yet have OOS ρ that collapses toward / below the null band — so the **OOS column is decisive**, and here only dims 1–2 survive it.

> Caveat carried throughout: in-sample ρ is an optimistic ceiling. The permutation null sits high precisely because J=37 is large and collinear; significance = observed clears that band.


## Per-dimension interpretation

| dim | OOS ρ | perm-p | top macro (struct corr) | bloc reading |
|---|---|---|---|---|
| dim1 | 0.641 | 0.001 | xle(-0.85), be_5y(-0.67), inflsw_5y(-0.66), fx_usdcad(+0.58) | fx(-0.79), equity(+0.76), infl(+0.65) |
| dim2 | 0.487 | 0.001 | fx_audusd(-0.67), fx_bbdxy(+0.65), fx_dxy(+0.64), fx_usdclp(+0.58) | infl(-0.65), rates(-0.52), fx(-0.51) |
| dim3 | 0.157 | 0.001 | tips_5y(+0.56), ust_5y(+0.52), ust_10y(+0.50), tips_10y(+0.49) | china(-0.66), rates(-0.62), credit(+0.27) |
| dim4 | 0.156 | 0.001 | fx_usdbrl(+0.69), ust_2y(-0.28), ust_10y(-0.24), ust_5y(-0.24) | vol(+0.52), freight(+0.35), rates(-0.32) |
| dim5 | 0.055 | 0.001 | ovx(+0.52), move(+0.44), mxwo(-0.41), spx(-0.34) | vol(+0.57), credit(+0.48), equity(-0.42) |
...
kcca_min ≫ canon_min ⇒ evidence of **nonlinear** shared structure beyond the linear map.
...
- [x] OOS rho computed for all 5 dims; perm null has n_perm draws — 
